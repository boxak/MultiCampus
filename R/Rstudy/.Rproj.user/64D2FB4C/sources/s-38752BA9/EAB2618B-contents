...
ft.3 <- function(x) invisible(x+10) #return함수의 종류, visible하지 않은 리턴
...
# is.vector는 참, is.list는 거짓이면 벡터형이 온것.
...
test1 <-function(p){ # p가 실질적으로 쓰이지 않기 때문에 argument안줘도 에러는 나지 않음.
  cat("난 수행함\n")
  testError(-1)
  cat("나 수행할 까요? \n")
}
