...
<% if(loginVO==null){ %>
	<button onclick="location.href='/springnews/resources/loginForm.html'">로그인하기</button>
<% 
}
else{
%>
	...
	function isAdmin(command){
		<% if(loginVO==null){ %>
			if(command=="insert"){
				...
				location.href = '/springnews/jobad?pgNum='+${sessionScope.pgNum};
			}
			<% if(vo!=null){ %>
			if(command=="update"){
				window.alert("로그인을 하셔야 글 수정을 하실 수 있습니다!");
				location.href = '/springnews/jobad?action=listone&post_id='+<%= vo.getPost_id() %>;
			}
		...
		%>
			if(command=="insert"){
				location.href = '/springnews/resources/jobadInsertForm.jsp';
			}
			<% if(vo!=null){ %>
			else if(command=="update"){
				<% if(loginVO.getMem_userid().equals(vo.getMem_userid())){ %>
					
					location.href = '/springnews/resources/jobadUpdateForm.jsp';
				...
				<% if(loginVO.getMem_userid().equals(vo.getMem_userid())){ %>
					...
					location.href = '/springnews/jobad?action=delete&post_id='+<%= vo.getPost_id() %>;
